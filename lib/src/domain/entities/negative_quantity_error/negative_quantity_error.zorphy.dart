...
class NegativeQuantityError {
  NegativeQuantityError({ErrorCode? this.errorCode, String? this.message});
  ...
  NegativeQuantityError copyWith({ErrorCode? errorCode, String? message}) {
  ...
  NegativeQuantityError copyWithNegativeQuantityError({
    ErrorCode? errorCode,
    String? message,
  }) {
  ...
  NegativeQuantityError patchWithNegativeQuantityError([
    NegativeQuantityErrorPatch? patchInput,
  ]) {
    final _patcher = patchInput ?? NegativeQuantityErrorPatch();
    final _patchMap = _patcher.patchMap;
    return NegativeQuantityError(
      errorCode: _patchMap.containsKey(NegativeQuantityError$.errorCode)
          ? (_patchMap[NegativeQuantityError$.errorCode] is Function)
                ? _patchMap[NegativeQuantityError$.errorCode](this.errorCode)
                : (_patchMap[NegativeQuantityError$.errorCode] is Patch)
                ? _patchMap[NegativeQuantityError$.errorCode].applyTo(
                    this.errorCode,
                  )
                : _patchMap[NegativeQuantityError$.errorCode]
          : this.errorCode,
      message: _patchMap.containsKey(NegativeQuantityError$.message)
          ? (_patchMap[NegativeQuantityError$.message] is Function)
                ? _patchMap[NegativeQuantityError$.message](this.message)
                : (_patchMap[NegativeQuantityError$.message] is Patch)
                ? _patchMap[NegativeQuantityError$.message].applyTo(
                    this.message,
                  )
                : _patchMap[NegativeQuantityError$.message]
          : this.message,
    );
  }
  ...
  bool operator ==(Object other) {
    ...
    return other is NegativeQuantityError &&
  ...
  Map<String, dynamic> toJsonLean() {
    final Map<String, dynamic> data = _$NegativeQuantityErrorToJson(this);
...
extension NegativeQuantityErrorPropertyHelpers on NegativeQuantityError {
...
extension NegativeQuantityErrorSerialization on NegativeQuantityError {
  Map<String, dynamic> toJson() {
    return _$NegativeQuantityErrorToJson(this);
  }
}

enum NegativeQuantityError$ { errorCode, message }

class NegativeQuantityErrorPatch
    extends PatchBase<NegativeQuantityError, NegativeQuantityError$> {
  NegativeQuantityError applyTo(NegativeQuantityError entity) {
    return entity.patchWithNegativeQuantityError(this);
  }

  NegativeQuantityErrorPatch withErrorCode(ErrorCode? value) {
    patchMap[NegativeQuantityError$.errorCode] = value;
    return this;
  }

  NegativeQuantityErrorPatch withMessage(String? value) {
    patchMap[NegativeQuantityError$.message] = value;
    return this;
  }
}

/// Field descriptors for [NegativeQuantityError] query construction
abstract final class NegativeQuantityErrorFields {
  static const errorCode = Field<NegativeQuantityError, ErrorCode?>(
    'errorCode',
    _$errorCode,
  );

  static const message = Field<NegativeQuantityError, String?>(
    'message',
    _$message,
  );

  static ErrorCode? _$errorCode(NegativeQuantityError e) {
    return e.errorCode;
  }

  static String? _$message(NegativeQuantityError e) {
    return e.message;
  }
}

extension NegativeQuantityErrorCompareE on NegativeQuantityError {
  Map<String, dynamic> compareToNegativeQuantityError(
    NegativeQuantityError other,
  ) {
